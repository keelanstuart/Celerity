...
DECLARE_RESOURCECODEC(Blobs);

c3::ResourceCodec::LoadResult RESOURCECODECNAME(Blobs)::ReadFromFile(c3::System *psys, const TCHAR *filename, const TCHAR *options, void **returned_data) const
{
	ResourceCodec::LoadResult ret = ResourceCodec::LoadResult::LR_ERROR;
...
c3::ResourceCodec::LoadResult RESOURCECODECNAME(Blobs)::ReadFromMemory(c3::System *psys, const TCHAR *contextname, const BYTE *buffer, size_t buffer_length, const TCHAR *options, void **returned_data) const
{
	if (returned_data)
	{
		BlobImpl *pblob = new BlobImpl();
...
bool RESOURCECODECNAME(Blobs)::WriteToFile(c3::System *psys, const TCHAR *filename, const void *data) const
{
	bool ret = false;
...
void RESOURCETYPENAME(Blob)::DestroyData(void *data) const
{
	((Blob *)data)->Release();
}
