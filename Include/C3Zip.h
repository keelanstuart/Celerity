...
#define ZFLAGS_HASENC	16
	...
	public:
		...
		ZIPERROR Error() { return error; }

		// Returns zip header block (for files opened in READ mode only!)
		const ZIPINFO *ZipInfo() const { return &header; }

		// Returns zip file info struct
		bool ZipFileInfo(size_t file_idx, ZIPFILEINFO *entry);

		// Finds a file in zip archive... places the starting address of the content in *content_addr and its length in content_len
		bool GetZipContent(const TCHAR *filename, void **content_addr = nullptr, size_t *content_len = nullptr);
	...
	private:
		...
		props::TFlags32 flags;			// Open flags for zip
		ZIPERROR error;					// Last zip error
		HANDLE m_hFile;					// Open file for zip file
		ZipEntryArray files;			// Files in zip
		TDirStructArray dirstructs;		// Directory struct array (used when writing)
		StringArray bufs;				// Memory buffers
		int bufnum, bufpos;				// Directory entry storage pos and size
		ZIPINFO header;					// Header of zip
	};
